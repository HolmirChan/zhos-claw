...
- **实现计划**: Task 5
- **子任务**:
  - [x] spawn.go async goroutine 回调前清空 result.BlockedType
  - [x] 编译 PASS
  - [x] 提交
...
### SB-005 · 阶段二：ToolResult BlockedType + facade re-export [x]
- **来源**: BL-011
- **实现计划**: Task 8, 12
- **子任务**:
  - [x] result.go 新增常量 + 字段 + WithBlockedType + ContentForLLM 前缀
  - [x] shared_facade.go + fs/shared.go 常量 re-export
  - [x] 单元测试：前缀拼装、空 content、普通错误无前缀
  - [x] 编译 + 测试 PASS
  - [x] 提交

### SB-006 · 阶段二：shell.go guardCommand 签名改造 [x]
- **来源**: BL-011
- **实现计划**: Task 9
- **子任务**:
  - [x] guardCommand 改为 (string, string)，6 处 return 全适配
  - [x] 3 处直接 ErrorResult 套 WithBlockedType
  - [x] 测试 PASS
  - [x] 提交

### SB-007 · 阶段二：filesystem.go sentinel + helper + Execute 层替换 [x]
- **来源**: BL-011
- **实现计划**: Task 10, 11
- **子任务**:
  - [x] ErrWorkspaceBoundary sentinel + 6 处挂载
  - [x] sandboxFs 3 条件拆分
  - [x] errorResultFromFS / errorResultFromFSCtx helper
  - [x] Execute 层 6 处 ErrorResult 替换
  - [x] 单元测试：sentinel 传播、OS 权限不触发
  - [x] 测试 PASS
  - [x] 提交
...
- **实现计划**: Task 13
- **子任务**:
  - [x] 全量测试 PASS（1 预存失败 + 1 文案更新已修复）
  - [x] make build && make build-launcher
  - [ ] 部署 RK3506 验证 LLM 自律停止 ≤ 3 次（需设备验证）
